...
RSpec.describe Services::Ping, '#call' do
  ...
  let(:result) { service.call(IPAddr.new(address)) }
  let(:rtt) { result[0] }
  ...
  context 'invalid address' do
    it 'returns zero rtt for broadcast address', address: '255.255.255.255' do
  ...
  context 'destination unreachable' do
    let(:timeout) { 1 }
    let(:result) { service.call(IPAddr.new(address), timeout) }
